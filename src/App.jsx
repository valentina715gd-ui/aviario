...
function App() {
  ...
  return (
    <div className="min-h-screen bg-cream page-grain pb-24 lg:pb-0">
      ...
      {showSettings && <SettingsForm profile={profile} onClose={() => setShowSettings(false)} onSave={async (nextProfile) => { let photoUrl = nextProfile.photo; if (supabase && session?.user && nextProfile.photoFile) { const path = `${session.user.id}/aviario-${Date.now()}`; const { error: uploadError } = await supabase.storage.from('fotos-aves').upload(path, nextProfile.photoFile, { upsert: true, contentType: nextProfile.photoFile.type }); if (uploadError) { showToast(uploadError.message.includes('Bucket not found') ? 'No se pudo guardar la foto. Pide al administrador activar las fotos.' : uploadError.message); return } photoUrl = supabase.storage.from('fotos-aves').getPublicUrl(path).data.publicUrl } if (supabase && session?.user) { const { data, error } = await supabase.from('aviarios').upsert({ user_id: session.user.id, nombre: nextProfile.name, foto_url: photoUrl || null, whatsapp: nextProfile.whatsapp || null, publicar_ventas: nextProfile.publish }, { onConflict: 'user_id' }).select().single(); if (error) { showToast(error.code === '42501' || error.message.includes('row-level security') ? 'No se pudo guardar el aviario. Pide al administrador revisar los permisos.' : error.message); return } setProfile({ ...nextProfile, photo: photoUrl, id: data.id }) } else setProfile({ ...nextProfile, photo: photoUrl }); setShowSettings(false); showToast('Perfil del aviario actualizado') }} />}
